...
interface Vehicle {
    ...
    toyotaRefreshToken?: string; // Encrypted refresh token
    toyotaUuid?: string;         // User UUID from id_token
    toyotaVin?: string;          // Linked Vehicle Identification Number
    toyotaLastSync?: Timestamp;  // Last successful sync time
    toyotaLastFuelLevel?: number; // Last known fuel level (%) to detect refueling
    toyotaLastMileage?: number;   // Last known mileage (km)
}
...
interface FuelLogData {
    ...
    isToyotaSynced?: boolean;   // Marks logs automatically generated by the Toyota integration
  }
